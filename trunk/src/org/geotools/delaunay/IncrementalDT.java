...
public class IncrementalDT {
			...
			for (int i = 0; i < 3; i++) {
				...
				if (Tni != null) {
					...
					if (delaunay(Tni, freePointB)) { 
						...
						for (int j = 0; j < 3; j++) { // storage of new neighbors
														// resulting triangle
							newTriangles.trian = triangles.getTriangle(newTriangles.trian.key);
							if (newTriangles.trian== null){
							return;
							}
	...
	private boolean delaunay(TriangleDT T, PointDT A) {
		...
		double N = ((Math.pow(T.C.x, 2) - Math.pow(T.A.x, 2)
				...
				* (T.C.x - T.A.x))
				/ (-2 * ((T.A.y - T.C.y) * (T.B.x - T.A.x) - (T.A.y - T.B.y)
						* (T.C.x - T.A.x)));
		if(N==Double.POSITIVE_INFINITY||N==Double.NaN||N==Double.NEGATIVE_INFINITY)			
		return false;			
		...
		if(M==Double.POSITIVE_INFINITY||M==Double.NaN||M==Double.NEGATIVE_INFINITY)
		return false;			
		...
		if(R==Double.POSITIVE_INFINITY||R==Double.NaN||R==Double.NEGATIVE_INFINITY)
		return false;			
	...
	public void closeDataStore() throws TreeException{
		triangles.closeDataStore();
	}
